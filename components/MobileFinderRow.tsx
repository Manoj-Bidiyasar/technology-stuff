...
export default function MobileFinderRow({ product }: { product: Product }) {
  const image = product.images?.[0] || "https://placehold.co/420x420?text=Phone";
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              <h3 className="line-clamp-2 text-xl font-extrabold text-slate-900">
                <Link href={`/mobile/${product.slug}`} className="hover:text-blue-700">
                  {product.name}
                </Link>
              </h3>
            ...
            <CompareToggleButton slug={product.slug} name={product.name} image={image} price={price} />
          </div>
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2">
            ...
            <div className="flex items-center gap-2">
              <Link href={`/mobile/${product.slug}`} className="text-sm font-bold text-slate-800 underline underline-offset-2">
                View All Specs
              </Link>
