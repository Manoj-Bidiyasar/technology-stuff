...
export default async function CompareSlugPage({ params }: CompareSlugProps) {
  ...
  const [products, trendingPool] = await Promise.all([
    getCompareProductsFromSlug(slug, "tablet"),
    listTrendingProducts(80, "tablet"),
  ]);
    ...
    <main className="mobile-container py-6 sm:py-8">
      <section>
        <CompareTable
          ...
          compareBasePath="/tablets/compare"
          specBasePath="/tablets"
          deviceType="tablet"
        />
      </section>
      ...
      <RelatedComparisons pairs={safeRelatedComparisons} initialVisible={14} compareBasePath="/tablets/compare" />
    </main>
  );
}
